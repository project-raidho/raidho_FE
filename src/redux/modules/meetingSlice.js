...
// ::: 게시글 리스트 출력
export const _getMeetingList = createAsyncThunk(
  "meeting/_getMeetingList",
...
const meetingSlice = createSlice({
  ...
  extraReducers: {
    // ::: 미팅리스트 전체 불러오기
    [_getMeetingList.pending]: (state, action) => {
      state.isLoading = true;
    },
    [_getMeetingList.fulfilled]: (state, action) => {
      state.isLoading = false;
      state.meetingList = action.payload;
    },
    [_getMeetingList.rejected]: (state, action) => {
      state.isLoading = false;
