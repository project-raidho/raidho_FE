...
interface ILocation {
  setTripLocation: Dispatch<SetStateAction<string[]>>;
}

function TripLocationSelect({ setTripLocation }: ILocation) {
  const area0 = [
    '시/도 선택',
    '서울특별시',
    '인천광역시',
  ...
  const sidohandleSelect = (e: SelectChangeEvent<string>) => {
    setSico(e.target.value);
    const index = area0.indexOf(e.target.value);
    setOptionlist(eval(`area${index}`));
  };

  const guhandleSelect = (e: SelectChangeEvent<string>) => {
  ...
  return (
    <div>
      <FormControl sx={{ m: 1, Width: 120 }}>
        <Select value={sido} onChange={sidohandleSelect} displayEmpty inputProps={{ 'aria-label': 'Without label' }}>
          <MenuItem value="">
            <em>시/도 선택</em>
          </MenuItem>
          {area0.map((item) => (
      ...
      <FormControl sx={{ m: 1, Width: 120 }}>
        <Select value={gu} onChange={guhandleSelect} displayEmpty inputProps={{ 'aria-label': 'Without label' }}>
          <MenuItem value="">
            <em>시/구/군선택</em>
          </MenuItem>
          {optionlist.map((item) => (
